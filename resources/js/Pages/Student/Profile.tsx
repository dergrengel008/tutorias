...
import type { StudentProfile, User as UserType } from '@/types';
...
export default function StudentProfileEdit({ profile, user }: PageProps) {
    const { props } = usePage();
    ...
    const handleFileChange = (
        field: 'selfie_image' | 'id_card_image',
        file: File | null
    ) => {
        if (file) {
            setData(field, file);
        ...
        axios
            ...
            .catch((error) => {
                if (error.response?.data?.errors) {
                    ...
                    setPwErrors({ general: error.response?.data?.message || 'Error al actualizar la contraseña.' });
                }
            })
            .finally(() => {
                    ...
                    <div className="bg-white rounded-xl shadow-lg p-6">
                        ...
                        <div className="flex items-center gap-6">
                            <div className="relative group">
                                {avatarPreview ? (
                                    <img src={avatarPreview} className="h-24 w-24 rounded-full object-cover border-4 border-indigo-100" alt="Foto de perfil" />
                                ) : (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input
                                    ...
                                    value={data.latitude}
                                    readOnly
                                    className="w-full rounded-lg border border-gray-300 bg-gray-50 px-4 py-2.5 text-gray-900"
